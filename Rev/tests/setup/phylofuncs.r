...
traj2tree <- function(traj) {
  
  # Start tree
  ntaxa <- sum(traj$n_sampled)
  phy <- list(edge=matrix(NA,nrow=2*ntaxa-2,ncol=2),tip.label=paste0("t",1:ntaxa),edge.length=numeric(2*ntaxa-2),Nnode=ntaxa-1)
  phy$edge[,2] <- c(1:ntaxa,(ntaxa+2):(2*ntaxa-1)) # reserve node ntaxa+1 for root
  
  # One time for every tip
  ages <- rep(traj$samp_times,traj$n_sampled)
  
  
  # Coalesce, adding serial samples to active taxa as we pass them
  active <- c(1:ntaxa)[ages == 0]
  to_activate <- 1:ntaxa
  to_activate <- to_activate[-active]
  for (i in (ntaxa-1):1) { # ensures node ntaxa+1 is the root
    
    # add this waiting time to all active edges/lineages
    phy$edge.length[phy$edge[,2] %in% active] <- phy$edge.length[phy$edge[,2] %in% active] + traj$intercoal_times[ntaxa - i]

    # Find tips that came active in this interval (and add appropriate duration to them)
    to_add <- intersect(which(ages < traj$coal_times[ntaxa - i]),to_activate)
    if ( length(to_add) > 0 ) {
      active <- c(active,to_add)
      to_activate <- to_activate[!(to_activate %in% to_add)]
      phy$edge.length[phy$edge[,2] %in% to_add] <- phy$edge.length[phy$edge[,2] %in% to_add] + (traj$coal_times[ntaxa - i] - ages[to_add])
      for (j in to_add) {
        phy$edge.length[phy$edge[,2] %in% active]
      }
    }
    
    # pick 2 to coalesce
    to_coalesce <- sample(active,2)

    # add ancestor to edge matrix
    phy$edge[phy$edge[,2] %in% to_coalesce,1] <- i+ntaxa
    
    # adjust active lineages
    active <- active[!(active %in% to_coalesce)]
    active <- c(active,i+ntaxa)
    
  }
  
  # Throw in matrix of tip times
  phy$Rev_taxon <- cbind(phy$tip.label,ages)
